...
def _unwrap_grouping(text: str) -> str:
    """Peel wrapping ``(…)`` and leftover unmatched trailing ``)``.

    Preprocessed 2-D accesses often look like ``(LOOKUP[i])[j]``. Blindly
    stripping a leading ``(`` leaves ``LOOKUP[i])``, which no longer ends
    in ``]`` so the subscript strip would fail.
    """
    s = (text or "").strip()
    changed = True
    while s and changed:
...
def _strip_trailing_subscripts(text: str) -> str:
    """Remove trailing ``[…]`` groups with bracket matching (not a split on '[').

    ``Foo[Controller_u8]`` → ``Foo``
    ``Foo[i][j]`` → ``Foo``
    """
    s = (text or "").strip()
    while s.endswith("]"):
...
def clean_indexed_object(raw: str) -> str:
    ...
    Nested subscripts (``Foo[i][j]`` / ``(Foo[i])[j]``) recurse after
    stripping a bracket-matched suffix so the lookup key is the base array.
    """
    text = _strip_leading_keyword((raw or "").strip())
    text = _unwrap_grouping(text)
    ...
    text = _unwrap_grouping(text)
    if "[" in text:
        stripped = _strip_trailing_subscripts(text)
        stripped = _unwrap_grouping(stripped)
        if stripped and stripped != text:
            return clean_indexed_object(stripped)
...
def extract_index_accesses(line_text: str) -> list[dict[str, Any]]:
    ...
    for rec in subscripts_from_text(line_text or ""):
        indexed = clean_indexed_object(str(rec.get("indexed") or ""))
...
def extract_nested_index_operands(line_text: str) -> tuple[str, list[str]]:
    """Base array name plus index expressions outer→inner for multi-dim access.

    ``Foo[i][j]`` → ``("Foo", ["i", "j"])`` using subscripts_from_text nesting.
    """
    recs = subscripts_from_text(line_text or "")
    if not recs:
        return "", []
    deepest = max(recs, key=lambda r: str(r.get("text") or "").count("["))
    raw_indexed = str(deepest.get("indexed") or "")
    base = clean_indexed_object(raw_indexed)
    if not base:
        return "", []
    # One nest chain only — a `!=` of two `Foo[i][j]` must not concatenate
    # both accesses into four index expressions.
    dcol = int(deepest.get("bracket_col") or 0)
    dtext = collapse_ws(str(deepest.get("text") or ""))
    ordered = sorted(recs, key=lambda r: int(r.get("bracket_col") or 0))
    indices: list[str] = []
    seen: set[tuple[int, str]] = set()
    for rec in ordered:
        rcol = int(rec.get("bracket_col") or 0)
        if rcol > dcol:
            continue
        rtext = collapse_ws(str(rec.get("text") or ""))
        if rtext and dtext and rtext not in dtext:
            continue
        idxed = str(rec.get("indexed") or "")
        cleaned = clean_indexed_object(idxed)
        if cleaned != base and not idxed.replace(" ", "").startswith(base):
            continue
        op = str(rec.get("operand") or "").strip()
        if not op or (rcol, op) in seen:
            continue
        seen.add((rcol, op))
        indices.append(op)
    return base, indices
...
class CaseFile:
    ...
    arg_read_helpers: list[str] = field(default_factory=list)
    ...
    def brief(self) -> str:
        ...
        if self.write_helpers:
            lines.append(
                "Helpers that write the operand before the alarm: "
                + ", ".join(self.write_helpers)
            )
        if self.arg_read_helpers:
            lines.append(
                "Callees that only read the operand (by value) — their internal "
                "checks apply to their own parameters, not automatically to this "
                "alarm's index: " + ", ".join(self.arg_read_helpers)
            )
        ...
        if self.guards:
            ...
            for g in self.guards[:12]:
                lines.append(f"  - {g}")
        lines.append(
            "Use get_micro_window (adapt window_size), run_simulation for raw bounds facts, "
            "inspect helpers when gaps exist, then submit_verdict when confidence is high."
        )
    ...
    *,
    ...
    for _ln, text in store.get_source_slice(start, end):
        ...
        for callee in callees_from_text(text):
            if (
                not callee
                or not callee.isidentifier()
                or callee in _SKIP_IDENT
                or callee in exclude
                or callee in seen
            ):
                continue
            # Write helpers, not token co-occurrence:
            #   Update(&checkword, core, tsk)          → write
            #   checkword = FirstCoreTsk()             → write
            #   GetTskIdx(checkword)                   → read (skip)
            #   ChkSingleU8(checkword, &result)        → read (skip)
            #   VConfirming(trcv, SlacCurProfileValidation) → read (skip)
            if not call_site_writes_operand(text, callee, operand_root):
                continue
            seen.add(callee)
            found.append(callee)
    return found
...
def _find_operand_arg_read_calls(
    store: DataStore,
    *,
    function_name: str,
    anchor_line: Optional[int],
    ...
    """Callees that take the operand by value on a line that mentions it."""
    if not function_name or not operand_root:
        return []
    start, end = None, None
    if anchor_line is not None:
    ...
    for _ln, text in store.get_source_slice(start, end):
        ...
        for callee in callees_from_text(text):
            if (
                ...
                continue
            if call_site_writes_operand(text, callee, operand_root):
                continue
            seen.add(callee)
            found.append(callee)
    return found
...
def _outparam_writes_in_span(
    ...
    *,
    ...
    """Call sites in [start, end] that pass ``&operand`` or assign from a callee."""
    if not operand_root or start is None or end is None:
        return []
    hi = end
    if before_line is not None:
        hi = min(end, before_line if inclusive else before_line - 1)
    hits: list[dict[str, Any]] = []
    for ln, text in store.get_source_slice(start, hi):
        if not token_in_text(text, operand_root):
            continue
        for callee in callees_from_text(text):
            if not callee or not callee.isidentifier() or callee in _SKIP_IDENT:
                continue
            if call_site_writes_operand(text, callee, operand_root):
                hits.append(
                    {
                        "line": ln,
                        "function": callee,
                        "assigned_expression_text": (text or "").strip()[:300],
                        "source": "outparam_call",
                    }
                )
                break
    return hits
...
def _writes_before(
    ...
    for hit in hits:
        ...
        writes.append(hit)
        seen_lines.add(ln)
    for hit in _outparam_writes_in_span(
        store, sanitize_symbol(symbol).split(".", 1)[0], start, end, before_line=before_line
    ):
        ln = hit.get("line")
        if ln in seen_lines:
            continue
...
def _collect_constant_inits(
    ...
    """Last assignment of the operand, including later out-param / global writes.

    A same-function ``token = 0`` must not hide ``Callee(&token)`` later in
    the same body (BswM queue pop) or a later assignment in another function.
    """
    ...
    root = sanitize_symbol(symbol).split(".", 1)[0]
    local_init = ""
    if fn == function and start is not None and end is not None:
        hits = store.find_source_assignments(
            symbol,
        ...
        if hits:
            text = (hits[-1].get("assigned_expression_text") or "").strip()
            if _looks_like_constant_init(text, tok):
                local_init = f"init in {function}: {text}"
        outparams = _outparam_writes_in_span(
            store, root, start, end, before_line=int(before_line), inclusive=True
        )
        # Prefer mentioning a later &token store over a lone constant init.
        later_out = [h for h in outparams if (h.get("line") or 0) >= (hits[-1].get("line") if hits else 0)]
        if local_init and later_out:
            callee = later_out[-1].get("function") or "callee"
            return [
                f"{local_init} (UNRESOLVED later write via {callee} — static "
                "search has no proof the runtime value stays in-bounds; do not "
                "treat the local init as a safety bound)"
            ]
        if not local_init and outparams:
            callee = outparams[-1].get("function") or "callee"
            return [
                f"UNRESOLVED origin write via {function}(...) — see {callee} "
                "(static search has no proof this stays in-bounds)"
            ]

    global_hits = store.find_source_assignments_global(
        symbol, before_line=int(before_line), max_hits=12
    )
    other = [
        h
        for h in global_hits
        if (h.get("function") or "") and h.get("function") != function
    ]
    if other:
        last = other[-1]
        other_fn = last.get("function") or "?"
        other_text = (last.get("assigned_expression_text") or "").strip()
        if local_init:
            return [
                f"{local_init} (UNRESOLVED later write via {other_fn} — static "
                "search has no proof the runtime value stays in-bounds; do not "
                "treat the local init as a safety bound)"
            ]
        if _looks_like_constant_init(other_text, tok):
            return [
                f"origin write via {function}(...) — see {other_fn}: {other_text} "
                "(cross-function; confirm the write is the value used at the alarm, "
                "not an earlier leftover init)"
            ]
        return [
            f"UNRESOLVED origin write via {function}(...) — see {other_fn} "
            "(static search has no proof this stays in-bounds)"
        ]
    return [local_init] if local_init else []


def _looks_like_bounds_guard(
    cond: str,
    token: str,
    alarm_line: Optional[int] = None,
    guard_line: Optional[int] = None,
) -> bool:
    """Keep comparisons/for-limits on the operand; drop the OOB access itself.

    The alarm line is never a bound on itself (``guard_line == alarm_line``),
    even when the condition text would otherwise pass condition_compares_token.
    """
    if (
        alarm_line is not None
        and guard_line is not None
        and int(alarm_line) == int(guard_line)
    ):
        return False
    text = cond or ""
    tok = (token or "").strip()
...
def _collect_guards(
    ...
    for tok in tokens:
        ...
        for g in data.get("guards_found") or []:
            ...
            gline = g.get("line")
            token_ok = any(
                _looks_like_bounds_guard(
                    text, t, alarm_line=alarm_line, guard_line=gline
                )
                for t in tokens
...
def compile_case(store: DataStore, order_id: int) -> CaseFile:
    ...
    nest_base, nest_dims = extract_nested_index_operands(alarm_text)
    nest_base = clean_indexed_object(nest_base) if nest_base else ""
    multi_dim = bool(nest_base) and "[" not in nest_base and "]" not in nest_base and len(nest_dims) >= 2
    if multi_dim:
        indexed_name = nest_base
        index_expr = "][".join(nest_dims)
    if not indexed_name:
    ...
    rank_src = " , ".join(nest_dims) if multi_dim and nest_dims else (index_expr or indexed_name)
    operand = rank_operand_symbol(
        operand_text=rank_src,
    ...
    reader_src = " , ".join(nest_dims) if multi_dim and nest_dims else index_expr
    index_reader_helpers = [
        h
        for h in callees_in_expression(reader_src)
        if h != operand and h != obj.name
    ]
    operand_root = sanitize_symbol(operand).split(".", 1)[0]
    exclude_helpers = {obj.name, operand, *index_reader_helpers}
    write_helpers = _find_operand_mutating_calls(
        store,
        function_name=alarm_fn,
        anchor_line=alarm_line,
        operand_root=operand_root,
        exclude=exclude_helpers,
    ) if operand_root else []
    arg_read_helpers = _find_operand_arg_read_calls(
        store,
        function_name=alarm_fn,
        anchor_line=alarm_line,
        operand_root=operand_root,
        exclude={*exclude_helpers, *write_helpers},
    ) if operand_root else []
    helpers = list(
        dict.fromkeys([*index_reader_helpers, *write_helpers, *arg_read_helpers])
    )
    gaps: list[str] = []
    if not indexed_name:
    ...
    elif multi_dim and obj.array_size is not None:
        gaps.append("multi_dim_partial")
