...
Triage Astrée **out-of-bounds** alarms. Python compiles the case (operand, object size, origin→alarm path, guards, and effective-bound hints) and **opens every path window**. The LLM only `inspect`s listed helpers and `submit_verdict`s `true` / `false` / `review`. Human CSV labels are never fed to the agent.
...
```text
Order id
  → compile_case (no LLM): object, operand, origin path (from the CF/DF graph),
                            Python guards, effective-bound hints
  → pack_path_windows: every path step is opened in Python,
                        in origin→alarm order (budget-capped,
                        [WINDOW TRUNCATED] marks any cut)
  → LLM: gets the case brief (object declaration + operand scope)
         and the packed windows before any tool call;
         may call get_window / move_window (walk the same graph-ordered
         path step-by-step) or inspect(helper) to re-open a step/helper —
         these always return the COMPLETE function body
...
The case brief now also carries a **Python-derived effective-bound hint** when the index is produced by a constant-origin sentinel walk of the indexed table. This prevents a specific false-positive reasoning pattern: an unchecked bit-pack / bit-extract helper does not by itself make the alarm `true` if the caller only enumerates valid entries until a sentinel or null terminator.

The LLM must consider the caller-path bound before submitting `true`. `submit_verdict` rejects a `true` rationale that only claims the helper is unchecked while ignoring the sentinel-bounded caller evidence. The model must either explain why the caller traversal does not actually bound the access, or submit `false` / `review`.

### Order 41 validation

Order 41 now emits the sentinel-bound hint. The previous unsupported `true` rationale is rejected by `submit_verdict`, while a `false` rationale that cites the caller traversal is accepted.

## Layout

`aoob_agent/` (compiler, tools, LangGraph), `cf_viz/` (WebGL + SSE), `scripts/batch_eval_20.py`.
